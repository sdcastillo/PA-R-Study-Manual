...
Let's create a histogram of the claims.  The first step is to create a blank canvas that holds the columns that are needed.  The library to make this is called [ggplot2](https://ggplot2.tidyverse.org/).  
...
The [boxplot](https://ggplot2.tidyverse.org/reference/geom_boxplot.html) compactly displays the distribution of a continuous variable. It visualises five summary statistics (the median, two hinges and two whiskers), and all "outlying" points individually.
...
```{block, type='studytip'}
**Shortcut:** This "mutate_if" trick can save you time from having to copy and paste from the SOA's template code and convert each variable to a factor individually
```
...
The most recent PA exams have had questions related to missing values.  You need to perform checks before moving on to the model-building phases.  In real-life, man-made data always has missing records.
...
- **Missing at random (MAR):** There is no pattern between the missingness and the value of the variable.
- **Missing not at random (MNAR):**  The value of the variable that is missing is related to the reason it is missing.  Example: A survey with regard to illegal drug use where respondents are reluctant to admit that they have broken the law.
- **Hidden missingness:** When value is coded incorrectly such as when a numeric variable that has “999” or “0” or a factor doesn’t match the Data Dictionary.

### Missing Value Resolutions:

You can use any of these methods.  Be sure to describe your reasoning in the report.
