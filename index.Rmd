--- 
title: "Predictive Analytics for Actuaries"
author: "Sam Castillo"
date: "1/19/2020"
github-repo: 
site: bookdown::bookdown_site
output: bookdown::gitbook
documentclass: book
classoption: openany
bibliography: [book.bib, packages.bib]
biblio-style: apalike
link-citations: yes
favicon: images/artificial_actuary_logo_favicon.png
---

# Preface {-} 

<img src="https://raw.githubusercontent.com/sdcastillo/ExamPAContent/master/book_cover.jpg" width="250" height="340" alt="Cover image" align="right" style="margin: 0 1em 0 1em" />Welcome!  This is the study guide for the SOA's [Predictive Analytics Exam](https://www.soa.org/education/exam-req/edu-exam-pa-detail/).  While meeting all of the learning requirements of Exam PA, this book gives you data science and machine learning training.  You will learn how to get your data into R, clean it, visualize it, and use models to derive business value.  Just as a scientist sets up lab experiments to form and test hypothesis, you’ll build models and then test them on holdout sets.  The statistics is just the first phase, as you’ll also learn how to explain the results in easy-to-understand, non-technical business language.

*We are thankful to all of the reviewers, guest editors, and past exam-takers who have helped to improve this book.  Thanks to the following persons who made changes to this book and its past versions: David Hill, Erlan Wheeler, Caden Collier, Peter Shelbe, Abhinav Gadde, Allen Meriken, [Kevin Kuo](https://github.com/kevinykuo), Aamir Ali, Matthew Caseras, and Liu Chang.*

# Join the online course

<font size="15"> Put studying in *fast forward* so you can plan your ASA celebration party `r emo::ji("party")` `r emo::ji("cake")` `r emo::ji("beer")` </font> 

Online course with proven success in December 2019 Exam PA, tutorial videos, and hands-on practice.

---

**"Teaching actuaries to train, validate, and explain their predictive models using plain English.**

**Machine learning should be as simple as possible, but not simpler.**

**- Sam Castillo, Predictive Modeler, Coach, Blogger."**

---

<img src="https://media-exp1.licdn.com/dms/image/C4E03AQGvNo3LIM7VpQ/profile-displayphoto-shrink_200_200/0?e=1595462400&v=beta&t=MdgKCnTFjA7erX4_H0qQAsW73qOTlhOnQkfK9gGrbOY" align="left" style="margin: 0 1em 0 1em">

With 3 years of predictive modeling experience at Milliman and Willis Towers Watson, Sam now coaches predictive modeling, maintains a [blog](http://sdcastillo.github.io) about the future of risk, and won the 2019 SOA Predictive Analytics and Futurism's [Jupyter contest](https://nbviewer.jupyter.org/github/SOASections/SOA-Predictive-Modeling-Innovation-and-Industry-Contest-2019-First-Place/blob/master/Predicting%20Uncertainty%20Prediction%20Intervals%20from%20Gradient%20Boosted%20Quantile%20Regression.ipynb).

<br></br>
<br></br>


<font size="6">**My manager didn’t think that I could pass both exams at once.**</font>

Let me show you how I passed exam PA with only about 80 study hours while also passing STAM in the same week.  This method worked for me and has worked for hundreds of other people.

Passing PA was possible by focusing on key aspects such as

•	Master communication – Imagine that this is English 101 rather than a math test.

•	Avoid technical difficulties that eat up your time.

•	Check your answers before the exam ends.

<br></br>
<br></br>

<font size="6">**Learn from my mistakes instead of yours.**</font>

The latest video collapses more than 1,000 study hours down to a single 3-hr video (300 hours for my initial sitting in June 2019, 500 hours when coaching other actuaries in December 2019, 200 hours writing the R study guide).  In total, I've been through each practice exam more than 6 times, each time under a 5-hour, and have encountered every mistake in the book!

<br></br>
<br></br>

<font size="6">**There's no need to be an R expert.**</font>

Most questions focus on comprehension and explanation.  While having R experience is certainly helpful, anyone can watch how I write answers in plain english.  With more than 6 years of R experience, my tutorials show you the cleanest way to get every answer, without getting lost in the weeds.


Our study packages features

- **Video solutions** to save you time `r emo::ji("clock")`
- **Original exams** for extra practice `r emo::ji("muscle")`
- **Cheat sheets** that give your brain a break `r emo::ji("sweat")`

<br></br>

```{block, type='rcorners'}
<font size="9">**[Sign Up](https://www.exampa.net/)**</font>
```




<br></br>
<br></br>

**COVID-19 Update**: In the event of a postponed June exam, all course members will receive an extension through the revised exam date.  Our Extended Access Guarantee gives you a free subscription if unsuccessful.  See the [Terms of Service](https://www.exampa.net/terms) for details.


# How to use this book

* Run the examples on your own machine by downloading the ExamPAData library
* Use the arrow keys to navigate the chapters
* Download this book:
  1) Go to the first chapter (https://sdcastillo.github.io/PA-R-Study-Manual/index.html)
  2) Right click anywhere on the page -> "Save As..."


# How to Contribute to this book

Your suggestions can be added within hours.  By submittine a pull request via Github, changes can be made to the R files which create this book.

<iframe src="https://player.vimeo.com/video/375173977?title=0&byline=0&portrait=0" width="640" height="360" frameborder="0" allow="autoplay; fullscreen" allowfullscreen></iframe></p>

**Contact:**

info@exampa.net


# The exam

The main challenge  of this exam is in communication: both understanding what they want you to do as well as telling the grader what it is that you did.

You will have 5 hours and 15 minutes to use RStudio and Excel to fill out a report in Word on a Prometric computer.  The syllabus uses fancy language to describe the topics covered on the exam, making it sound more difficult than it should be.  A good analogy is a job description that has many complex-sounding tasks, when in reality the day-to-day operations of the employee are far simpler.

A non-technical translation is as follows:

**Writing in Microsoft Word (30-40%)**

- Write in professional language
- Type more than 50 words-per-minute

**Manipulating Data in R (15-25%)**

- Quickly clean data sets
- Find data errors planted by the SOA
- Perform queries (aggregations, summaries, transformations)

**Machine learning and statistics (40-50%)**

- Interpret results within a business context
- Change model parameters

Follow the SOA's page for the latest updates

https://www.soa.org/education/exam-req/edu-exam-pa-detail/

The exam pass rates are about 50%.

http://www.actuarial-lookup.com/exams/pa

# Prometric Demo

The following video from Prometric shows what the computer set up will look like.  In addition to the files shown in the video, they will give you a printed out project statement (If they don't give this to you right away, ask for it.)

<iframe src="https://player.vimeo.com/video/304653968" width="640" height="360" frameborder="0" allow="autoplay; fullscreen" allowfullscreen></iframe>
<p><a href="https://vimeo.com/304653968">SOAFinalCut</a> from <a href="https://vimeo.com/user10231556">Prometric</a> on <a href="https://vimeo.com">Vimeo</a>.</p>

https://player.vimeo.com/video/304653968

# Introduction

While "machine learning" is relatively new, the process of learning itself is not.   All of use are already familiar with how to learn - by improving from our mistakes.  By repeating what is successful and avoiding what results in failure, we learn by doing, by experience, or trial-and-error.  Machines learn in a similar way.

Take for example the process of studying for an exam.  Some study methods work well, but other methods do not.  The "data" are the practice problems, and the “label” is the answer (A,B,C,D,E).  We want to build a mental "model” that reads the question and predicts the answer.

We all know that memorizing answers without understanding concepts is ineffective, and statistics calls this "overfitting".  Conversely, not learning enough of the details and only learning the high-level concepts is "underfitting".

The more practice problems that we do, the larger the training data set, and the better the prediction.  When we see new problems, ones which have not appeared in the practice exams, we often have a difficult time. Quizing ourselves on realistic questions estimates our preparedness, and this is identical to a process known as "holdout testing" or "cross-validation". 

We can clearly state our objective: get as many correct answers as possible! We want to correctly predict the solution to every problem.  Said another way, we are trying to minimize the error, known as the "loss function".  

Different study methods work well for different people.  Some cover material quickly and others slowly absorb every detail.  A model has many "parameters" such as the "learning rate".  The only way to know which parameters are best is to test them on real data, known as "training".
