...
The other link functions are not straight-forward to interpret using math.  One solution is to use the **model-demo-method.**
...
**SOA Exam PA, December 12, 2019, task 11**
...
The answer asks you to run the example cases through your model to calculated the predicted probability of each case.  You need to create the column **Prob of high** using your GLM.  Then you assign each case as being "High" or "Low" depending on if this value is above the cutoff.
...
- Having never married decreases profitability
...
Then we set `x = TPR` and `y = FNR` and graph x against y.  The plot below shows the ROC for the `auto_claims` data.  The Area Under the Curve of 0.6795 is what we would get if we integrated under the curve.
