...
Characters can be combined with the `paste()` function.
...
Booleans are just `TRUE` and `FALSE` values.  R understands `T` or `TRUE` in the same way,
but the latter is preferred.  When doing math, bools are converted to 0/1 values where 1 is equivalent to TRUE and 0 FALSE.
...
```{r}
ls <- list(bool = TRUE, character = "character", numeric = 3.14)
ls
```
...
# The pipe is from the magrittr library
library(magrittr) 
max(c(3, 4, 16)) %>% 
  sqrt() %>% 
  log2() %>% 
